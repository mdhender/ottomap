...
// schemaMap is the on-disk XML root. It tracks only the attributes and
// children we need for round-tripping through the ottomap domain model.
// Many Worldographer-specific attributes (display flags, factors, grid
// numbering) are decoded into the struct but not propagated to the domain.
