...
// Encode renders m as a UTF-8 XML document targeting the Worldographer 2017
// (Hexographer) schema family.
func Encode(m *ottomap.Map, opts Options) ([]byte, error) {
	...
	if orientation == "ROWS" {
		tilesWide, tilesHigh = rows, cols
	} else {
		tilesWide, tilesHigh = cols, rows
	}
...
func writeHeader(b *strings.Builder, orientation, projection string, gmView bool) {
	fmt.Fprintf(b,
		`<?xml version="1.0" encoding="utf-8"?>`+"\n"+
			`<map type="WORLD" version="1.77" lastViewLevel="WORLD" `+
			...
			`showNotes="true" showGMOnly="%t" showGMOnlyGlow="false" showFeatureLabels="true" `+
