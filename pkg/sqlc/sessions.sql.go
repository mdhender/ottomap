...
func (q *Queries) ReadSession(ctx context.Context, sid string) (ReadSessionRow, error) {
	row := q.db.QueryRowContext(ctx, readSession, sid)
	var i ReadSessionRow
	err := row.Scan(&i.Uid, &i.ExpiresAt)
	return i, err
}

const readSessions = `-- name: ReadSessions :many
SELECT sid, uid, expires_at
FROM sessions
WHERE CURRENT_TIMESTAMP < expires_at
`

func (q *Queries) ReadSessions(ctx context.Context) ([]Session, error) {
	rows, err := q.db.QueryContext(ctx, readSessions)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
